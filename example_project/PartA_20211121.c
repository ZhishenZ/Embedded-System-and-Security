...
int main(void) {
  ...
  while(1){
	for(int32_t waiter=(1<<18); waiter >= 0; waiter--);
	if(state) tmp++;
	else tmp--;
	if(tmp>=99) state=0;
	else if (tmp<=1) state=1;
	_pwm(tmp);
  }
  return 0;
}
...
void initCCU4() {
  ...
  //step 3, Enable the CCU4 clock via the specific SCU register, CLKSET
  SCU_CLK->CLKSET = SCU_CLK_CLKSET_CCUCEN_Msk;

  //step 4, Enable the prescaler block
...
int _pwm(uint32_t dutyC){
	...
	CCU40_CC42->CRS = idc;  //duty cycle set
	CCU40->GCSS = CCU4_GCSS_S2SE_Msk;	
